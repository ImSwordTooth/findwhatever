export const CHANGELOG_DATA = [
	{
		...
		contentList: [
			'全新架构：纯内存文本投影模型彻底取代 cloneNode，检索性能与稳定性提升一个数量级',
			'现代标准：可见性探测升级为原生 checkVisibility API，彻底杜绝 DOM 建树重排',
			'去 antd 化：浮窗完全由原生精简组件构建，包体积与运行时开销大幅缩减',
			'动画库统一：彻底拔除 framer-motion，全插件规范迁移至 motion/react 弹簧动效',
			'多帧加固：iframe 探测优化为批量高效 IPC，杜绝 0x0 隐藏 iframe 伪匹配',
			'样式净化：解决高亮样式表重复插入漏洞，关闭或切换时幂等清理',
			'细节打磨：重构最近搜索建议流，支持键盘滚轮平滑跟随与 0 贴边坐标记忆'
		]
	},
	{
		version: 'v4.1.0',
