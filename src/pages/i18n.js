export const i18n = (text) => {
	...
	switch (text) {
		...
		case '填入并开启正则模式': return 'Restore and enable regex mode'
		case '固定之': return 'Pin'
		case '取消固定': return 'Unpin'
		case '暂无数据': return 'Empty'
		case '为了避免输入正则表达式的过程中卡死，开启此选项后的输入防抖会持续 1 秒': return 'To avoid the input of regular expressions from freezing, the search will automatically be aborted after 1 second'
		...
		case '重置成功': return 'Reset successfully'
