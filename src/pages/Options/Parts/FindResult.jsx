...
export const FindResult = () => {
	...
	return (
		<div>
			<div className="areaTitle mt-[30px]">{t('查找结果')}</div>
			<div>
				...
				<div>{t('v3.0 新增了跨标签搜索。启用跨标签搜索方案的条件为：')}</div>
				<ol style={{ margin: '0' }}>
					<li>{t('没有嵌套结构')}</li>
					<li>{t('子节点中至少包含这些标签其中的一个：')}['STRONG','WBR','EM', 'ABBR', 'A', 'SPAN', 'ADDRESS', 'B', 'BDI', 'BDO', 'CITE', 'I', 'KBD', 'MARK', 'Q', 'S', 'DEL', 'INS', 'SAMP', 'SMALL', 'SUB', 'SUP', 'TIME', 'U', 'VAR']</li>
					<li>{t('没有换行')}</li>
					<li>{t('子节点长度大于 1，')}{'<div><a>1111</a></div>'} {t('这种结构，没必要规范化')}</li>
				</ol>
				<div><em>{t('核心且复杂的功能，如果有搜索结果错误的页面，请及时联系我，谢谢！')}</em></div>
