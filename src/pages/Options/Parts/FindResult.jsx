	...
	return (
		<div>
			...
			<div className="space-y-8">
				...
				<div>
					<p className="mb-2.5">
						{t('展示查找结果(search-results)的数量和当前定位到的结果的下标(search-results-active)。')}
					</p>
					<p className="mb-2.5 flex items-center flex-wrap gap-1">
						<span>{t('点击')}</span>
						<span className="inline-flex items-center cursor-grab active:cursor-grabbing hover:text-[var(--swe-color-primary)] transition-colors px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-xs">
							<span className="scale-90 origin-right">{t('查找结果')}</span>
							<CopySvg className="w-2.5 h-2.5 ml-1" />
						</span>
						<span>{t('，会把结果复制到剪贴板。')}</span>
					</p>

					<p className="mb-2.5">{t('v3.0 新增了跨标签搜索。启用跨标签搜索方案的条件为：')}</p>
					<ol className="list-decimal list-inside space-y-1 mb-2.5 text-zinc-600 dark:text-zinc-400 text-xs">
						<li>{t('没有嵌套结构')}</li>
						<li className="break-all">{t('子节点中至少包含这些标签其中的一个：')}<code>['STRONG','WBR','EM', 'ABBR', 'A', 'SPAN', 'ADDRESS', 'B', 'BDI', 'BDO', 'CITE', 'I', 'KBD', 'MARK', 'Q', 'S', 'DEL', 'INS', 'SAMP', 'SMALL', 'SUB', 'SUP', 'TIME', 'U', 'VAR']</code></li>
						<li>{t('没有换行')}</li>
						<li>{t('子节点长度大于 1，')}<code className="px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 font-mono text-[11px]">&lt;div&gt;&lt;a&gt;1111&lt;/a&gt;&lt;/div&gt;</code> {t('这种结构，没必要规范化')}</li>
					</ol>
