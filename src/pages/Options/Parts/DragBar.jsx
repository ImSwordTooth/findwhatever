...
export const DragBar = () => {
	...
	return (
		<div>
			<div className="areaTitle mt-[30px]">{t('拖拽条')}</div>

			<div>
				<div>{t('点击并拖拽此处可调整面板位置，会自动记忆位置。')}</div>
				<div>{t('如果因为修改了浏览器窗口宽高导致面板位置异常（如修改窗口大小、打开控制台等），会在下一次打开时自动临时重置位置。')}</div>
				<div>{t('如果超过了设备的宽高，会在下一次打开时自动重置位置并清除记忆的位置。')}</div>
			</div>
